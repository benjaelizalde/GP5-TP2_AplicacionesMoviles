...
public final class CiudadDao_Impl implements CiudadDao {
  ...
  @Override
  public Object contarCiudadesPorPais(final String pais,
      final Continuation<? super Integer> $completion) {
    final String _sql = "SELECT COUNT(*) FROM ciudades WHERE nombrePais = ?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    if (pais == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindString(_argIndex, pais);
    }
    final CancellationSignal _cancellationSignal = DBUtil.createCancellationSignal();
